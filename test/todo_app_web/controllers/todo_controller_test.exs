defmodule TodoAppWeb.TodoControllerTest do
  ...
  describe "index" do
    test "lists all entries on index", %{conn: conn, user: user, todo: todo} do
      conn = get(conn, Routes.user_todo_path(conn, :index, user))
  ...
  describe "show todo" do
    test "shows chosen todo if it belongs to current_user", %{conn: conn, user: user, todo: todo} do
      conn = get(conn, Routes.user_todo_path(conn, :show, user, todo))
    ...
    test "returns errors when current_user is nil", %{user: user, todo: todo} do
      conn =
        ...
        |> get(Routes.user_todo_path(build_conn(), :show, user, todo))
    ...
    test "returns errors when todo does not belong to current_user", %{conn: conn} do
      ...
      conn = get(conn, Routes.user_todo_path(conn, :show, other, 3))
    ...
    test "returns nil when id is nonexistent", %{conn: conn, user: user} do
      conn = get(conn, Routes.user_todo_path(conn, :show, user, 10))
  ...
  describe "create todo" do
    test "creates and returns todo when data is valid", %{conn: conn, user: user} do
      conn = post(conn, Routes.user_todo_path(conn, :create, user), todo: @create_attrs)
    ...
    test "does not create todo and returns errors when data is invalid", %{conn: conn, user: user} do
      conn = post(conn, Routes.user_todo_path(conn, :create, user), todo: @invalid_attrs)
      assert json_response(conn, 422)["errors"]["body"] == ["can't be blank"]
    end
  end

  describe "update todo" do
    test "updates and returns chosen todo when data is valid", %{
      conn: conn,
      user: user,
      todo: todo
    } do
      conn = put(conn, Routes.user_todo_path(conn, :update, user, todo), todo: @update_attrs)
    ...
    test "does not update chosen todo when data is invalid", %{conn: conn, user: user, todo: todo} do
      conn = put(conn, Routes.user_todo_path(conn, :update, user, todo), todo: @invalid_attrs)
  ...
  describe "delete todo" do
    test "deletes chosen todo", %{conn: conn, user: user, todo: todo} do
      conn = delete(conn, Routes.user_todo_path(conn, :delete, user, todo))
